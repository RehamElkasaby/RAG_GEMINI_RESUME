...
from cv_schema import MatchResult
...
def export_cv_data_to_excel(cv_data_dict: Dict[str, Any]) -> bytes:
    """Export CV data to Excel format"""
    try:
        from io import BytesIO
        
        # Create DataFrame
        df = create_cv_dataframe(cv_data_dict)
        
        # Create Excel file in memory
        output = BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='CV_Summary', index=False)
            ...
            for filename, cv_data in cv_data_dict.items():
                ...
                if skills_data:
                    ...
                    sheet_name = f"{filename[:20]}_Skills"  # Limit sheet name length
                    skills_df.to_excel(writer, sheet_name=sheet_name, index=False)
                
                # Experience sheet
                experience_data = cv_data.get('experience', [])
                if experience_data:
                    exp_df = pd.DataFrame(experience_data)
                    sheet_name = f"{filename[:20]}_Experience"
        ...
        st.error("openpyxl package is required for Excel export")
        return None
...
def highlight_matched_text(text: str, keywords: List[str]) -> str:
